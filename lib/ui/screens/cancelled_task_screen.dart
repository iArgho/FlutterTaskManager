...
class _CancelledTaskScreenState extends State<CancelledTaskScreen> {
  bool _getCancelledTaskScreen = false;
  TaskListModel _taskListModel = TaskListModel();

  Future<void> getCompletedTask() async {
    _getCancelledTaskScreen = true;
    if (mounted) {
      setState(() {});
    }
    final NetworkResponse response =
    await NetworkCaller().getRequest(Urls.inCancelledTask);
    if (response.isSuccess) {
      _taskListModel = TaskListModel.fromJson(response.body!);
    } else {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('In cancelled tasks get failed')));
      }
    }
    _getCancelledTaskScreen = false;
    if (mounted) {
      setState(() {});
    }
  }
